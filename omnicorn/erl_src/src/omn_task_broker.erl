...
init([]) ->
    ...
    lists:foreach(fun({omn_activities, Id, Name, Payload, _Retries}) ->
...
%% Add this clause to handle_info to support the delayed boot retry
handle_info({retry_task, Id, Payload}, S) ->
    case omn_router:checkout_worker() of
        {ok, W} ->
            omn_worker:send_async(W, #{<<"type">> => <<"activity_execute">>, <<"payload">> => Payload#{<<"activity_id">> => Id}});
        _ ->
